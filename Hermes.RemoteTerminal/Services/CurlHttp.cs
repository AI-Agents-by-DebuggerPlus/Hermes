...
/// <summary>
/// HTTPS via curl.exe (OpenSSL). Fallback when HttpClient/Schannel fails (same stack as RemoteTerminal.Xp).
/// </summary>
internal static class CurlHttp
{
        ...
        try
        {
            ...
            foreach (var c in candidates)
            {
                if (File.Exists(c))
                {
                    ...
                    AppLog.Info("REST backup: curl transport " + _resolved);
                    return;
                }
            }
            ...
            foreach (var dir in pathEnv.Split(';', StringSplitOptions.RemoveEmptyEntries))
            {
                try
                {
                    ...
                    {
                        _resolved = p;
                        AppLog.Info("REST backup: curl transport (PATH) " + p);
                        return;
                    }
                }
                catch
        ...
        int timeoutSec)
    {
        ...
        try
        {
            if (body is not null)
            {
                bodyFile = Path.Combine(Path.GetTempPath(), "hrt_" + Guid.NewGuid().ToString("N") + ".json");
            ...
            var psi = new ProcessStartInfo
            {
                ...
                CreateNoWindow = true,
                WorkingDirectory = Path.GetDirectoryName(_resolved) ?? AppContext.BaseDirectory,
            };
