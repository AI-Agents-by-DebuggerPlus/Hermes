...
public static class ChatBehaviorDefaults
{
    ...
    /// <summary>Default reply length for Hermes chat (all modes).</summary>
    public const string ConciseReplyRu =
        "Длина ответа: по умолчанию **2–3 предложения** по сути текущего вопроса, без markdown-списков и без длинных обзоров. "
        + "Развёрнутый ответ — только если пользователь явно просит подробности "
        + "(«подробнее», «разверни», «объясни детально», «полный разбор» и аналоги).";
    ...
    public const string HermesWpfClientCapabilitiesRu =
        ...
        + "Навык **показаний воды (Reni Water / водоканал)**: фразы «передай показания», «отправь завтра показания», «статус водоканала» обрабатываются локально в Hermes.Wpf "
        + "(Playwright + scripts/reni_water); показания считываются с сайта автоматически, ввод цифр не нужен.";
}
